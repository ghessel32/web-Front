...
function SpeedFix({ fixData, deviceType }) {
  ...
  const getCustomDescription = (type, items = []) => {
    ...
    switch (type) {
      case "Image Optimization":
        return `Found ${count} unoptimized image${count !== 1 ? "s" : ""} totaling approximately ${(count * 0.3).toFixed(1)} MB.`;
      case "Render-Blocking Requests":
        return `${count} file${count !== 1 ? "s" : ""} block rendering and delay first paint.`;
      case "Font Loading Strategy":
        return `${count} web font${count !== 1 ? "s" : ""} are loading inefficiently and may delay text rendering.`;
      case "Third-Party Code":
        return `${count} external script${count !== 1 ? "s" : ""} from third-party sources are affecting page performance.`;
      default:
        return `Detected ${count} performance-related issue${count !== 1 ? "s" : ""} in this category.`;
    }
  };
  ...
  const IssueItem = ({ icon: Icon, issue }) => {
    ...
    return (
      <div
        ...
        className={`p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-all duration-200 border border-white/10 cursor-${isClickable ? "pointer" : "default"}`}
      >
        <div className="flex items-start sm:items-center">
          ...
          <div className="ml-3 sm:ml-4 flex-1">
            <h4 className="text-sm sm:text-base font-semibold text-gray-200 mb-1">{type}</h4>
    ...
    <>
      <div className="bg-white/5 backdrop-blur-xl rounded-2xl p-6 shadow-2xl border border-white/10 hover:border-white/20 transition-all duration-300">
        <div className="flex items-center mb-4 sm:mb-6">
          ...
          <h3 className="text-lg sm:text-xl font-semibold text-white ml-3 sm:ml-4">
        ...
        <div className="space-y-3 sm:space-y-4">
          {issues.length > 0 ? (
            ...
            <div className="text-sm sm:text-base text-gray-400 text-center py-4">
        ...
        <div className="mt-6 p-4 rounded-xl bg-gradient-to-br from-orange-500/10 to-red-500/10 border border-orange-500/20">
          <div className="flex items-center justify-between text-sm sm:text-base">
            <span className="text-gray-300">Potential Speed Improvement</span>
            ...
            {selectedIssue.items && selectedIssue.items.length > 0 ? (
              <ul className="max-h-[70vh] overflow-y-auto p-5 space-y-2 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent">
                {selectedIssue.items.map((item, i) => (
                  <li
                    key={i}
                    className="text-sm sm:text-base text-gray-300 bg-white/5 hover:bg-white/10 p-3 rounded-xl border border-white/10 transition-all duration-200"
                  >
                    <a
                      href={item.url}
                      target="_blank"
